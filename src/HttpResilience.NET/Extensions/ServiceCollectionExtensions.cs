...
public static class ServiceCollectionExtensions
{
    ...
    private sealed class HttpResilienceOptionsValidator : IValidateOptions<HttpResilienceOptions>
    {
        ...
        public ValidateOptionsResult Validate(string? name, HttpResilienceOptions options)
        {
            ...
            if (options.Connection.MaxConnectionsPerServer is < 1 or > 1000)
    ...
    /// <param name="builder">The HttpClient builder.</param>
    /// <param name="configuration">Configuration containing "HttpResilienceOptions" section.</param>
    /// <param name="requestTimeoutSeconds">Total request timeout in seconds; when null, uses <see cref="TimeoutOptions.TotalRequestTimeoutSeconds"/>.</param>
    ...
    /// to configure the inner resilience pipeline (e.g. AddTimeout, AddRetry, AddCircuitBreaker, AddRateLimiter) in any order.
    /// </param>
        ...
        Action<IHttpClientBuilder>? configurePipeline = null)
    {
        ...
        _ = requestTimeoutSeconds ?? options.Timeout.TotalRequestTimeoutSeconds;
        ...
        AddCustomStandardHandler(builder, fallbackHandler, configureInnerPipeline, configurePipeline);
        return builder;
    }
