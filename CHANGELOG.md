...
- `**PipelineType`** toggle (`Standard` / `Hedging`) driven entirely from configuration.
- `**PipelineOrder**` and `**PipelineStrategyOrder**` for controlling outer strategy ordering without code changes.
...
- Unit tests (`HttpResilience.NET.Tests`) and integration tests (`HttpResilience.NET.IntegrationTests`).
